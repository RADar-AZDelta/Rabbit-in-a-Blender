...
class CreateCdmFolders(EtlBase, ABC):
    ...
    def run(self) -> None:
        ...
        for omop_table, table_props in vars(self._omop_tables).items():
            ...
            query_path = cast(Path, self._cdm_folder_path) / omop_table / "example_query.sql"
            ...
            for concept_column in (column for column in columns if "concept_id" in column):
                ...
                usagi_csv_path = cast(Path, self._cdm_folder_path) / omop_table / concept_column / "example_usagi.csv"
